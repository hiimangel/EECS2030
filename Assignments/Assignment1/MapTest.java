...
public class MapTest {
	...
	private static boolean isPathCorrect(String path, int startX, int startY, int destX, int destY) {
		...
		while (token.hasMoreTokens()) {
			...
			System.out.print(x + " isoathcorrect x, ");
			System.out.println(y + " ispathcorrect y");
			System.out.print(startX + " isoath startx ");
			System.out.println(startY + " ispath starty");
	...
	private static boolean toBorder(String path, int row, int col) {
		...
		System.out.println(x + ":     toBorder X");
		System.out.println(y + ":     toBorder Y");
	...
	public void testFindPath1() {
		...
		System.out.println(toBorder(actualPath, n1, n2) + " toBorder function");
		System.out.println(findDuplication(actualPath) + " findDuplication function");
		System.out.println(isPathCorrect(actual, startX, startY, destX, destY) + " isPathCorrect function");
	...
	public void testFindPath2() {
		...
		boolean correct = toBorder(actualPath, n1, n2) && findDuplication(actualPath) && isPathCorrect(actual, startX, startY, destX, destY);
		assertTrue(correct);
	}
	@Test
	public void testFindPath3() {
		...
		String point = findBorder(actualPath);
		String actual = (actualPath.substring(actualPath.indexOf(')') + 1)).trim();
		System.out.println(actualPath + "maptest file");
		int destX = Integer.parseInt(point.substring(point.indexOf('(') + 1, point.indexOf(','))) ;
		int destY = Integer.parseInt(point.substring(point.indexOf(',') + 1 , point.indexOf(')')));
	...
	public void testFindPath4() {
		...
		Map city = new Map (n1, n2);
		String actualPath = city.findPath (startX, startY);
		System.out.println(actualPath + " maptest file actualPath");
		String point = findBorder(actualPath);
		String actual = (actualPath.substring(actualPath.indexOf(')') + 1)).trim();
		int destX = Integer.parseInt(point.substring(point.indexOf('(') + 1, point.indexOf(','))) ;
		int destY = Integer.parseInt(point.substring(point.indexOf(',') + 1 , point.indexOf(')')));
	...
	public void testFindPath6() {
		...
		int destX = Integer.parseInt(point.substring(point.indexOf('(') + 1, point.indexOf(','))) ;
		int destY = Integer.parseInt(point.substring(point.indexOf(',') + 1 , point.indexOf(')')));
		Boolean toborder = toBorder(actualPath, n1, n2);
		Boolean finddup = findDuplication(actualPath);
		Boolean ispath = isPathCorrect(actual, startX, startY, destX, destY);
		System.out.println(toborder);
